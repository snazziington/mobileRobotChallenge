...
with Picamera2() as camera:
    ...
    while True: # Runs until key is pressed to close
        ...
        if object_identified == True:
            ...
            fg = backSub.apply(obj_cropped_img, learningRate = learning_rate)
            #cv2.imshow("fg", fg)
        ...
        if object_identified == True:
            ...
            if centerXO >= width / 2 + 50:
                turn_timer = time.time()
                while time.time() - turn_timer < interval_turn:
                    fc.turn_left(speed)
                fc.stop()

            elif centerXO <= width / 2 - 50:
                turn_timer = time.time()
                while time.time() - turn_timer < interval_turn:
                    fc.turn_right(speed)
                fc.stop()

            if difference_in_distance > 50: 
                print("object far")
                fc.forward(speed)

            elif difference_in_distance < -50: 
                print("object close")
                fc.backward(speed)

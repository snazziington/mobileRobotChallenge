...
import tty
import termios
from hitmarkerObjectDetection import HitmarkerObjectDetector
...
hitmarker_color_threshold = 20
object_hitmarker_padding = 10
avoidance_bias_strength = 2.0
avoidance_max_bias = 35

object_detector = HitmarkerObjectDetector(width, height, horizon)

def hitmarker_inside_box(hitmarker, x, y, w, h, padding=0):
        hitmarker_x, hitmarker_y = hitmarker
        return (
            x - padding <= hitmarker_x <= x + w + padding
            and y - padding <= hitmarker_y <= y + h + padding
        )

def remove_object_hitmarkers(active_hitmarkers, x, y, w, h):
        return [
            hitmarker
            for hitmarker in active_hitmarkers
            if not hitmarker_inside_box(hitmarker, x, y, w, h, object_hitmarker_padding)
        ]

def obstacle_steering_bias(obstacle_hitmarkers):
        grid_center_x = (object_detector.columns + 1) / 2
        bias = 0

        for hitmarker in obstacle_hitmarkers:
            grid_x, grid_y = object_detector.hitmarker_grid_position(hitmarker)
            side_strength = grid_center_x - grid_x
            closeness = grid_y / object_detector.rows
            bias += side_strength * closeness * closeness

        return float(np.clip(bias * avoidance_bias_strength, -avoidance_max_bias, avoidance_max_bias))

def robot_movement(centerXO, centerYO, object_held_distance, obstacle_hitmarkers=None):
        obstacle_hitmarkers = obstacle_hitmarkers or []
        ...
        avoidance_bias = obstacle_steering_bias(obstacle_hitmarkers)
        steering_angle = object_angle + avoidance_bias
        ...
        #else:
        turning_speed = min(80, np.interp(abs(steering_angle), [15, 50], [1, 5]))
        turning_speed += min(20, len(obstacle_hitmarkers) * 2)
        ...
        print("Avoidance bias", avoidance_bias)
        print("Steering angle", steering_angle)
        if obstacle_hitmarkers:
            print("Obstacle hitmarkers:", object_detector.format_hitmarkers(obstacle_hitmarkers))
        ...
        elif steering_angle < -(object_angle_margin):
...
with Picamera2() as camera:
    ...
    while True: # Runs until key is pressed to close
        ...
        active_hitmarkers = object_detector.color_changed_hitmarkers(frame, hitmarker_color_threshold)
        obstacle_hitmarkers = active_hitmarkers
        ...
        if object_identified == True:
            ...
            obstacle_hitmarkers = remove_object_hitmarkers(active_hitmarkers, xO, yO, wO, hO)
            object_detector.draw_hitmarkers(frame, confirmed_hitmarkers=obstacle_hitmarkers)
            cv2.rectangle(frame, (xO, yO), (xO + wO, yO + hO), magenta, 1)
            ...
            robot_movement(centerXO, centerYO, object_held_distance, obstacle_hitmarkers)
        else:
            object_detector.draw_hitmarkers(frame, confirmed_hitmarkers=active_hitmarkers)

        # Places line at the horizon (for our reference)
        cv2.line(frame, (0, horizon), (width, horizon), blue, 1)
        object_detector.draw_floor_sample_box(frame)
        frame = cv2.flip(frame, 1)
